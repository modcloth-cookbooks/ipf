...
cloudflare_ipv4_ips = node[:ipf][:ip_ranges].gsub(/\n/," ").split
...
template "/etc/ipf/ipf.conf" do
  source "cloudflare-ipf.conf.erb"
  owner "root"
  mode "0644"
  variables(:cloudflare_ipv4_ips => cloudflare_ipv4_ips.uniq, :private_ip_ranges => private_ip_ranges)
